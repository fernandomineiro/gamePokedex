...
const Explore: React.FC = () => {
  ...
  async function loadPokemons() {
    ...
    if (pokeUrl) {
      try {
        setIsLoading(true);
        const { data } = await axios.get<any>('https://pokeapi.co/api/v2/pokemon?limit=151');
        const mapped = data.results?.map((result:any) => {
        ...
        setPokeURL('https://pokeapi.co/api/v2/pokemon/ditto' || "");
      } catch (error) {
        console.error(error);
      }
      setIsLoading(false);
    ...
    <>
      <T.Container style={{ marginBottom: navHeight }}>
        ...
        {!isLoading ? (
          pokeUrl && (
            <T.Footer>
              <Button onClick={() => loadPokemons()}>Carregar mais</Button>
            </T.Footer>
          )
        ) : (
